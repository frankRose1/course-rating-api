...
describe('middleware', () => {
  ...
  describe('checkPagination', () => {
    it('should throw a HTTP400Error if client sends invalid pageSize in the querystring', () => {
      expect(() => mockCheckPagination('I should be a number!')).toThrow(HTTP400Error);
      expect(() => mockCheckPagination('I should be a number!'))
        .toThrow(/^If providing a page size please ensure the value is a number\.$/);
    });
    ...
    it('should throw a HTTP400Error if client sends invalid pageNum in the querystring', () => {
      ...
      expect(() => mockCheckPagination(1, 'wrongSize'))
        .toThrow(/^If providing a page number please ensure the value is a number\.$/);
    });
  })
